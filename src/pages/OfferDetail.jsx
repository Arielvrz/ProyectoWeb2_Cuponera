...
import { isValidCardNumber, isValidExpiry, isValidCVV } from "../utils/validation"
...
export default function OfferDetail() {
  ...
  useEffect(() => {
    const loadOffer = async () => {
      ...
      try {
        ...
        if (!data) { setError("La oferta no existe o ya no esta vigente."); return; }
        setOffer({
          id: data.id,
          title: data.titulo,
  ...
  const canPay = useMemo(() => {
    ...
    if (!isValidCVV(form.cvv)) return false
  ...
  const handlePay = async (e) => {
    ...
    setError("")
    setSuccess("")
    if (!isAuthenticated || !session?.access_token) { navigate("/login"); return; }
    try {
      setProcessing(true)
      const createdCodes = []
      ...
      setSuccess(`Compra exitosa. Se generaron ${createdCodes.length} cupon(es).`)
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="p-8">
              <div className="text-xs uppercase tracking-wider text-[var(--color-textMuted)] mb-2">
                {offer.rubro} &bull; {offer.companyName}
              </div>
              <h1 className="text-4xl font-extrabold text-white leading-tight">{offer.title}</h1>
              <p className="mt-4 text-[var(--color-textMain)] opacity-90">{offer.description}</p>
              <div className="mt-6 flex items-end gap-4">
                <div className="text-4xl font-extrabold text-white">{offer.priceOffer}</div>
                <div className="text-[var(--color-textMuted)] line-through text-sm font-medium">{offer.priceRegular}</div>
              </div>
              <div className="mt-4 text-sm text-[var(--color-textMuted)]">
                Limite para usar cupon:{" "}
        ...
        <div className="bg-[var(--color-darkSurface)] border border-gray-800 rounded-sm shadow-[0_0_25px_rgba(0,0,0,0.35)]">
          <div className="p-8">
            <div className="flex items-center justify-between gap-4 flex-wrap">
              ...
              <button
                type="button"
                onClick={() => setForm({ name: "Juan Andres Juarez", card: "4111 1111 1111 1111", exp: "12/29", cvv: "123" })}
            ...
            {loadingOffer && <div className="mt-6 border border-gray-700 bg-black/30 text-white p-4 rounded-sm">Cargando detalle de oferta...</div>}
            {error && <div className="mt-6 border border-[var(--color-accentRed)]/50 bg-black/30 text-white p-4 rounded-sm">{error}</div>}
            {success && <div className="mt-6 border border-green-500/50 bg-black/30 text-white p-4 rounded-sm">{success}</div>}
            ...
            <form onSubmit={handlePay} className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="md:col-span-2">
                ...
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  className="mt-2 w-full bg-[var(--color-darkBg)] border border-gray-800 px-4 py-3 rounded-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-accentRed)]/60"
                  placeholder="Juan Andres Juarez" />
              </div>
              <div className="md:col-span-2">
                <label className="text-sm text-[var(--color-textMuted)]">Numero de tarjeta</label>
                <input value={form.card} onChange={(e) => setForm({ ...form, card: e.target.value })}
                  className="mt-2 w-full bg-[var(--color-darkBg)] border border-gray-800 px-4 py-3 rounded-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-accentRed)]/60"
                  placeholder="4111 1111 1111 1111" inputMode="numeric" />
                <div className="mt-2 text-xs text-[var(--color-textMuted)]">Prueba rapida: 4111 1111 1111 1111</div>
              </div>
              <div>
                <label className="text-sm text-[var(--color-textMuted)]">Expiracion (MM/YY)</label>
              ...
              <div>
                ...
                <input type="password" value={form.cvv} onChange={(e) => setForm({ ...form, cvv: e.target.value })}
                  className="mt-2 w-full bg-[var(--color-darkBg)] border border-gray-800 px-4 py-3 rounded-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-accentRed)]/60"
                  placeholder="123" inputMode="numeric" />
              </div>
              <div className="md:col-span-2 flex gap-3 flex-wrap pt-2">
                <button disabled={!canPay || processing}
                  ...
                  type="submit">
                  {processing ? "Procesando..." : "Pagar y generar cupon"}
                </button>
              ...
              <div className="md:col-span-2 text-xs text-[var(--color-textMuted)] pt-2">
                Esto es una simulacion (sin cobro real). El cupon se guarda en Supabase ligado a tu usuario.
              </div>
            </form>
          </div>
        </div>
